(ns n-gram.words.hierarchical-dirichlet (:require [n-gram.misc.misc-functions :refer :all]
...
(defn F-w2-given-w1  "Returns all counts of bigrams with word w2 as the second word" [w2] (zipmap (map key counts-2)
(map #(if (= (second (key %)) w2) (val %) 0.0) counts-2)))
...
(defn P ([w2 w1 alpha m] (let [F-w2-w1 (get-count-memo counts-2 [w2 w1]) F-w1 (get-count-memo counts-1 [w1]) m-i (find-m-i-memo [w2] m) the-f-w2-w1 (f-w2-w1-memo F-w2-w1 F-w1) lambda-w1 (lambda-memo F-w1 alpha)]
                    (+ (* lambda-w1 m-i) (* (- 1 lambda-w1) the-f-w2-w1))))
([w2 w1] (let [F-w2-w1 (get-count-memo counts-2 [w2 w1]) F-w1 (get-count-memo counts-1 [w1]) m-i (find-m-i-memo [w2] m-optimum) the-f-w2-w1 (f-w2-w1-memo F-w2-w1 F-w1) lambda-w1 (lambda-memo F-w1 alpha-optimum)]
                   (+ (* lambda-w1 m-i) (* (- 1 lambda-w1) the-f-w2-w1)))))

(def P-memo (memoize P))

(defn perplexity ([alpha m] (Math/pow (reduce * (map #(P (second %) (first %) alpha m) pairs)) (- (/ 1 N))))
([] (Math/pow (reduce * (map #(P (second %) (first %) alpha-optimum m-optimum) pairs)) (- (/ 1 N)))))
