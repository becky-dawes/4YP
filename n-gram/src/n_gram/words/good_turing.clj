...
(def n-r-counts-keys-1 "1-gram map of counts-of-counts and indices with counts as keys" 
  (zipmap sorted-r-1 cumsum-counts-of-counts-1))

(def n-r-index-keys-1 "1-gram map of counts-of-counts and indices with indices as keys"
...
(def sorted-r-2 "2-gram n-r sequence" (generate-sorted-r-sequence-memo counts-of-counts-2))

(def n-r-counts-keys-2 "2-gram map of counts-of-counts and indices with counts as keys" 
  (zipmap sorted-r-2 cumsum-counts-of-counts-2))

(def n-r-index-keys-2 "2-gram map of counts-of-counts and indices with indices as keys"
...
(def sorted-r-3 "3-gram n-r sequence" (generate-sorted-r-sequence-memo counts-of-counts-3))

(def n-r-counts-keys-3 "3-gram map of counts-of-counts and indices with counts as keys" 
  (zipmap sorted-r-3 cumsum-counts-of-counts-3))

(def n-r-index-keys-3 "3-gram map of counts-of-counts and indices with indices as keys" 
...
(defn n-r-plus-one "Returns count of next biggest count" [freq n] 
  (let [n-r-index (get-count-memo (resolve (symbol (str "n-r-counts-keys-" n))) freq)] 
    (if (= n-r-index (count (var-get (resolve (symbol (str "counts-of-counts-" n)))))) 
      (n-r-memo freq n) ((resolve (symbol (str "counts-of-counts-" n))) 
                          ((resolve (symbol (str "n-r-index-keys-" n))) (+ n-r-index 1))))))
