...
(defn find-letter-pair "Finds all letter pairs starting with given letter" 
  ([letter] (zipmap (map key all-char-pair-counts) (map #(if (= (str(first (key %))) letter)
...
(defn find-letter-4 "Finds all letter groups of 4 starting with given letter trio" 
  ([l1 l2 l3] (zipmap (map key all-char-4-counts) (map #(if (and (= (str(first (key %))) l1) 
                           (= (str(second (key %))) l2) (= (str (nth (key %) 2)) l3))(val %) 0.0) all-char-4-counts)))
  ([l1 l2 l3 the-map] (zipmap (map key the-map) (map #(if (and (= (str(first (key %))) l1) 
                             (= (str(second (key %))) l2)(= (str (nth (key %) 2)) l3))  (val %) 0.0) the-map))))
...
(defn next-letter "Predicts next letter in sequence" 
  ([letters] (let [letters (clojure.string/lower-case letters)] 
               (cond (= (count letters) 1) (str (last (key (apply max-key val (find-letter-pair-memo (str (last letters)))))))
                            (= (count letters) 2) (str (last (key (apply max-key val (find-letter-trio-memo (vector_to_string (butlast letters)) 
                                                                                                   (str (last letters)))))))
...
(defn predict-text "Predicts a certain length of text based on context"
  ([context n] (if (< 0 n) (str context
                                (loop-next-letters (str (last context)
                                                 (next-letter-memo (str (last (vector_to_string (butlast context))) (str (last context))))) (- n 2)))))
