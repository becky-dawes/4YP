...
The CRP is implemented in our prefix tree representation of the training corpus as a Multi-Floor CRP (MFCRP). This is a hierarchical distribution. Seating a customer at a new table means drawing from the parent restaurant (i.e. creating a new table based on the CRP probabilities in the base distribution).
...
\item \lstinline!depth!: a vector containing the start and end depth of the branch's context (based on an uncoagulated prefix trie)
\end{itemize}
...
{:range [],
 :children
 {"h" {:range [0 2], :children {}, :restaurant {"e" [1 1]}, :depth [1 2]},
  "t" {:range [0 1], :children {}, :restaurant {"h" [1 1]}, :depth [1 1]},
  "" {:range [0 0], :children {}, :restaurant {"t" [1 1]}, :depth [0 0]}},
 :restaurant {"e" [1 1], "h" [1 1], "t" [1 1]},
 :depth [0 0]}
\end{lstlisting}
\doublespacing
...
\begin{tikzpicture}[node distance = 4cm, auto,>=triangle 45]
 ...
 \node [block, left of=match_range_check,node distance=5.4cm] (branch) {Branch node into two sections and instantiate table for last symbol of prefix};
...
The function \lstinline!branch! is used to branch a node when a context matches part of the branch substring. It dissociates the current node from the tree (line 9) and then associates a new node with the desired new range, as well as associating a child node that is a copy of the old node, but with a context that is only the second part of the original range.
...
\noindent It then recursively calls itself, removing one character from the beginning of \lstinline!word! until the tree for the final character is constructed. This then becomes the new \lstinline!root_node!. This  continues for each prefix, ensuring that the tree is built up from the shortest prefix first. This function gives a full tree for the word ``mississippi" as shown in Figure \ref{fig:mississippiCodePrefixTree}.
...
{:range [], :children
 {"p" {:range [8 9], :children
   {"p" {:range [0 9], :children {}, :restaurant {"i" [1 1]}, :depth [2 10]},
    "i" {:range [0 8], :children {}, :restaurant {"p" [1 1]}, :depth [2 9]}},
   :restaurant {"i" [1 1], "p" [1 1]}, :depth [1 1]},
  "i" {:range [1 2], :children
   {"s" {:range [1 4], :children
     {"s" {:range [0 4], :children {}, :restaurant {"p" [1 1]}, :depth [5 8]},
      "m" {:range [0 1], :children {}, :restaurant {"s" [1 1]}, :depth [5 5]}},
     :restaurant {"p" [1 1], "s" [1 1]}, :depth [2 4]},
    "m" {:range [0 1], :children {}, :restaurant {"s" [1 1]}, :depth [2 2]}},
   :restaurant {"p" [1 1], "s" [1 2]}, :depth [1 1]},
  "s" {:range [2 3], :children
   {"s" {:range [1 3], :children
     {"s" {:range [0 4], :children {}, :restaurant {"i" [1 1]}, :depth [4 7]},
      "m" {:range [0 1], :children {}, :restaurant {"i" [1 1]}, :depth [4 4]}},
     :restaurant {"i" [1 2]}, :depth [2 3]},
    "i" {:range [1 2], :children
     {"s" {:range [0 4], :children {}, :restaurant {"s" [1 1]}, :depth [3 6]},
      "m" {:range [0 1], :children {}, :restaurant {"s" [1 1]}, :depth [3 3]}},
     :restaurant {"s" [1 2]}, :depth [2 2]}},
   :restaurant {"i" [1 1], "s" [1 1]}, :depth [1 1]},
  "m" {:range [0 1], :children {}, :restaurant {"i" [1 1]}, :depth [1 1]},
  "" {:range [0 0], :children {}, :restaurant {"m" [1 1]}, :depth [0 0]}},
 :restaurant {"p" [1 2], "s" [1 2], "i" [1 3], "m" [1 1]}, :depth [0 0]}
 \end{lstlisting}
\doublespacing
...
Once the tree has been built, prediction is rather straightforward. Given a particular context, one must search through the tree for a match. If a match cannot be found, then a node may be instantiated, either by extending a branch, or by fragmenting an existing branch. As mentioned earlier, the probability of choosing an existing table is $\frac{n_{k}-d}{n+\alpha}$, where $n_{k}$ is the number of customers seated at that table, $d$ and $\alpha$ are the discount parameter and concentration parameter (zero in this project) respectively and $n$ is the total number of customers in the restaurant. The probability of choosing a new table (i.e. drawing a symbol from the parent distribution) is $\frac{\alpha+Nd}{n+\alpha}$, where $N$ is the number of tables in the restaurant. Drawing from the parent distribution means repeating this process again at the next highest node. 

Discount parameters vary depending on a node's \textit{depth} in the tree. In this project, the values given in \textit{A Stochastic Memoizer for Sequence Data} \cite{wood2009stochastic} were used. These are $d_{[0,1,2,...]}=(0.62, 0.69, 0.74, 0.80, 0.95, 0.95, ...)$. In this situation, \textit{depth} actually refers to the depth in a prefix \textit{trie}, that is a tree in which no coagulation has taken place. Figure \ref{fig:prefixTrie} shows the trie for ``mississippi". When coagulation takes place, the discount parameters must be multiplied together in accordance with the following theorem: 
\\
