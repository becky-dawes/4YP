...
The Sequence Memoizer uses a hierarchical model. However, unlike the model in Chapter \ref{chap:HierarchicalDirichletModel}, it uses Pitman-Yor processes (not Dirichlet processes). These are described in more detail in Section \ref{sec:PitmanYor}.
...
The Pitman-Yor process (PYP), $G=\{G(s)\}_{s\in\Sigma}$, has three parameters. These are the base distribution, $G_{0}=\{G_{0}(s)\}_{s\in\Sigma}$ (mean of PYP and prior on frequency of each symbol), the discount parameter, $d$, which is between 0 and 1 (affects exponent of power law), and the concentration parameter, $c$, (affects variability around $G_{0}$). When $d=0$, the PYP becomes the Dirichlet process. A PYP with discount parameter $d$ and prior $G_{0}$ is written $G\sim\mathcal{PY}(d,G_{0})$ (we assume in this project that $c=0$).
...
The Sequence Memoizer goes a step further than the above equations, using hierarchical PYPs (HPYP). We denote the empty context $\epsilon$ and the parent of a context $\boldsymbol{u}$ (i.e. $\boldsymbol{u}$ with the first symbol from the left removed) as $\sigma(\boldsymbol{u})$. These give Equations \ref{eq:SM4a}-\ref{eq:SM4c}. These state that:

\begin{itemize}
\item The empty context has prior and discount  $G_{0}$ and $d_{0}$ respectively (Equation \ref{eq:SM4a})
\item The context $\boldsymbol{u}$ has prior and discount $G_{\sigma(\boldsymbol{u})}$ and $d_{|\boldsymbol{u}|}$ respectively (Equation \ref{eq:SM4b})
\item Each symbol $x_{i}$ given that the previous $n$ symbols $x_{i-n:i-1}$ are the context $\boldsymbol{u}$ is distributed $G_{\boldsymbol{u}}$ (Equation \ref{eq:SM4c})
\end{itemize}

\begin{subequations}
\label{eq:SM4}
\begin{align}
G_{\epsilon}&~\sim\mathcal{PY}(d_{0},G_{0}) \label{eq:SM4a}
\\
G_{\boldsymbol{u}}|G_{\sigma(\boldsymbol{u})}&\sim\mathcal{PY}(d_{|\boldsymbol{u}|},G_{\sigma(\boldsymbol{u})})\ \ \ \ \text{for all }\boldsymbol{u}\in\frac{\Sigma_{n}^{*}}{\epsilon} \label{eq:SM4b}
\\
...
A new class of data type was created to store the information required for each node object: \lstinline!(defrecord restaurant_node [range children restaurant depth])!, where the fields are defined as follows:
...
The function \lstinline!branch! is used to split a node when a context matches part of the branch substring. It dissociates the current node from the tree and then associates a new node with the desired new range, as well as associating a child node that is a copy of the old node, but with a context that is only the second part of the original range (see Figure \ref{fig:branching}).

\begin{figure}[H]
\centering
\begin{subfigure}{0.3\textwidth}
\Tree [.$\bullet$ m im sim ]
\caption{Before Branching}
\label{fig:branchingA}
\end{subfigure}
\begin{subfigure}{0.3\textwidth}
\Tree [.$\bullet$ m im [.s im ] ]
\caption{After Branching}
\label{fig:branchingB}
\end{subfigure}
\begin{subfigure}{0.3\textwidth}
\Tree [.$\bullet$ m im [.s im sim ] ]
\caption{Adding New Context}
\label{fig:branchingC}
\end{subfigure}
\caption{Branching a Node}
\label{fig:branching}
\end{figure}
...
\item \lstinline!params!: the parameters describing the node being built on (for example,  \lstinline![:children "a"]! refers to the child of the root node whose range begins with the letter ``a"; \lstinline![:children "a" :range]! refers to the range of that node) 
\end{itemize}

\noindent It then recursively calls itself, removing one character from the end of \lstinline!word! until the tree for the first character has been constructed. This then becomes the new \lstinline!root_node! onto which the tree for the next longest prefix is built. This  continues for each prefix, ensuring that the tree is built up from the shortest prefix first. 
