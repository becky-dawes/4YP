...
\label{fig:treeFlowchart}
\end{figure}

Figure \ref{fig:treeFlowchart} details the tree building procedure. The cycle continues until the longest suffix has been constructed. A main \lstinline!build_tree! function was written with several auxiliary functions supporting it. 

Checking whether a node's range matches a given context was done using regular expressions. It is necessary to discern between full and partial matches with both the range and the context, so a function  \lstinline!check_range! was written which returns a vector of the form \lstinline![matching_string! \newline \lstinline!is_match_length_equal_to_context_length is_context_length_greater_than_match_length]!.
...
\noindent It then recursively calls itself, removing one character from the beginning of \lstinline!word! until the tree for the final character is constructed. This then becomes the new \lstinline!root_node!. This  continues for each suffix, ensuring that the tree is built up from the shortest suffix first.
