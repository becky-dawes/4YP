...
Similarly to Chapter \ref{chap:HierarchicalDirichletModel}, we employ the use of hierarchical processes. In this case, these are hierarchical PYPs (HPYP). We denote the empty context as $\epsilon$ and the parent of a context $\boldsymbol{u}$ (i.e. $\boldsymbol{u}$ with the first symbol from the left removed) as $\sigma(\boldsymbol{u})$. These give Equations \ref{eq:SM4a}-\ref{eq:SM4c}. These state that:

\begin{itemize}
\item The empty context has prior and discount  $G_{0}$ and $\alpha_{0}$ respectively (Equation \ref{eq:SM4a})
\item The context $\boldsymbol{u}$ has prior and discount $G_{\sigma(\boldsymbol{u})}$ and $\alpha_{|\boldsymbol{u}|}$ respectively (Equation \ref{eq:SM4b})
